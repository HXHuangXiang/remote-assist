...
- 屏幕采集走 DXGI Desktop Duplication,锁屏桌面回退 GDI BitBlt。
- H.264 编码用 Media Foundation Transform(系统自带,零外部依赖),浏览器端 WebCodecs 解码。
- 鉴权:WebSocket 第一帧 {"t":"auth","token":"<密码>"};密码 SHA-256(salt) 哈希存配置,首次启动随机生成并在托盘展示一次。
...
管理员 CMD 执行 tools/install.bat(需与 RemoteAssist.exe 同目录),或手动:
    ...
    sc create RemoteAssist binPath= "<绝对路径>\RemoteAssist.exe --service" start= auto obj= LocalSystem
    sc description RemoteAssist "RemoteAssist 远控被控端"
    sc start RemoteAssist

卸载:

    sc stop RemoteAssist
    sc delete RemoteAssist
...
MVP 只支持:单显示器主屏、单客户端连接、明文 HTTP+密码(局域网信任环境)。
不做:文件传输、剪贴板、NAT 穿透、TLS、UAC Secure Desktop 注入、Ctrl+Alt+Del 注入、多客户端并发。

## 备注

- 此项目复刻了附件 PoC 的核心链路:Windows Service(LocalSystem) -> 复制 winlogon.exe token -> CreateProcessAsUser 启动 agent -> OpenInputDesktop/SetThreadDesktop -> 采集与 SendInput。在此之上增加了 DXGI/GDI 采集、Media Foundation H.264 编码、cpp-httplib WebSocket 推流与浏览器 WebCodecs 控制端。
- 键盘走 scancode 路径(KEYEVENTF_SCANCODE),对 Winlogon/锁屏桌面更兼容。
- Media Foundation H.264 encoder 与 DXGI 的个别 API 标识符在 Windows SDK 不同版本间偶有差异,首次编译若个别符号未定义,通常是 1-2 行的名称修正。
