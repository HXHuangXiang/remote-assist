...
运行日志位于 exe 同级的 logs/：service.log 记录服务与子进程拉起，agent.log 记录采集、编码与网络，并在有控制端时每 10 秒输出一次推流统计；tray.log 与 setup.log 分别记录本地界面。画质在网页工具栏选择：自动、原始清晰度或固定 1080p/720p/540p/360p。固定模式锁定分辨率，Agent 仍会为可操作性自适应帧率和码率；网页还可设置局部更新阈值（10%–90%，默认 50%）。这两项偏好仅保存到当前浏览器，不写入被控端配置。性能字段的判读与普通桌面/锁屏回归步骤见 [性能诊断](/docs/performance-diagnostics.md)。
