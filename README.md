...
运行日志位于 exe 同级的 logs/：service.log 记录服务与子进程拉起，agent.log 记录采集、编码与网络，并在有控制端时每 10 秒输出一次推流统计；tray.log 与 setup.log 分别记录本地界面。画质在网页工具栏选择：自动、原始清晰度或固定 1080p/720p/540p/360p。网页还可直接输入帧率（1–60 FPS）和码率（0.1–6.25 MB/s，1 MB/s = 8 Mbps），并从常用预设中选择；这些数值固定当前控制端的帧率和码率，自动画质仍可调整分辨率。局部更新阈值可选 10%–90%，默认 50%。所有网页推流偏好仅保存到当前浏览器，不写入被控端配置；断开后 Agent 恢复服务端默认的自适应参数。性能字段的判读与普通桌面/锁屏回归步骤见 [性能诊断](/docs/performance-diagnostics.md)。
