...
管理员 CMD 执行 tools/install.bat(需与 RemoteAssist.exe 同目录),或直接双击 exe。双击时会
自动请求 UAC 管理员权限，随后可在窗口中设置密码并点击“安装/更新并启动”。已有服务时该按钮会更新 SCM 的映像路径并自动重启服务，使新 exe 立即生效。两种方式都会检查 exe、web/、已有 config.json、.config.lock、logs/
和遗留 .initial-password 与父目录链的 ACL：只要存在普通用户可写的路径、重解析点，或
LocalSystem 对运行所需资源权限不足，就会拒绝注册服务；不会移动配置、日志或网页资源。
请将整个 dist 放在仅管理员和 LocalSystem 可写的本机固定磁盘目录。手动 `sc create`
会绕过该检查，若确有需要，请先执行 `RemoteAssist.exe --check-service-install-dir`。

升级时不要直接覆盖正在运行的服务映像。请先停止服务再替换同目录文件，或把新版
`RemoteAssist.exe` 与 `web/` 放入新的安全目录并从新版双击进入配置窗口，然后点击
“安装/更新并启动”；配置文件与日志仍保留在该新版 exe 所在目录。
...
运行日志位于 exe 同级的 logs/：service.log 记录服务与子进程拉起，agent.log 记录采集、编码与网络，并在有控制端时每 10 秒输出一次推流统计；tray.log 与 setup.log 分别记录本地界面。配置窗口可设“画质上限”（自动、1080p、720p、540p）；它是自适应分辨率可恢复到的上限，机器或网络过载时仍会继续降档。性能字段的判读与普通桌面/锁屏回归步骤见 [性能诊断](/docs/performance-diagnostics.md)。
