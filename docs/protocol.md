...
1. 进入流模式先发一帧配置:
   ...
   {"t":"cfg","codec":"avc1.42E01E","annexb":true,"w":1920,"h":1080,"fps":30,"stream_id":1,
    "quality":"auto","patch_threshold":50,"patches":true}
...
3. 支持局部更新的网页会先在 `stream` 控制消息中声明 `patches:true`。当 DXGI/GDI
   ...
   图块批次：
       ...
       {"t":"tile","id":43,"i":0,"x":64,"y":128,"w":64,"h":64}
       <紧随其后的 JPEG 二进制负载>
       {"t":"tile","id":43,"i":1,"x":128,"y":128,"w":64,"h":64}
       <紧随其后的 JPEG 二进制负载>

   图块坐标相对于 `cfg` 画面。网页必须按顺序合成全部图块后才回传一次
   `{"t":"ack","id":43}`；同一时刻服务端只允许一个图块批次在途。任一图块
   图块批次和启用局部协议后的完整基线都采用 1 秒确认时限；解码失败、确认超时、
   切屏或流版本变化都会放弃旧批次并从新的完整关键帧恢复。
   未声明能力的旧网页只接收原有完整帧协议。
...
所有文本 JSON:
    ...
    {"t":"stream","quality":"720p","patch_threshold":50,"patches":true}
...
必须是 10 到 90 间、步进为 5 的整数。该偏好只在当前连接有效，网页可自行在本机保存；
断连后 Agent 恢复自动完整帧模式。
